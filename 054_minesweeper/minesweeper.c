...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t *mboard = malloc(sizeof(*mboard));
  mboard->width = w;
  mboard->height = h;
  mboard->totalMines = numMines;
  mboard->board = malloc(h * sizeof(*mboard->board));

  for(int i = 0; i < h; i++){
     mboard->board[i] = malloc(w * sizeof(*mboard->board[i]));
  }
  
  for(int i = 0; i < h; i++){
    for(int j = 0; j < w; j++){
      mboard->board[i][j] = UNKNOWN;
    }
  }

  for(int i = 0; i < numMines; i++){
    addRandomMine(mboard);
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  for(int dx = -1; dx < 2; dx++){
	...
	continue;
      ...
      if(x+dx >=0 && x+dx < b->width && y+dy >=0 && y+dy < b->height && IS_MINE(b->board[y+dy][x+dx])){
	count++;
      }
    }
  }
